...
<style>
    ...
    .judge-name { font-size: 1.15rem; font-weight: 850; color: #0f172a; letter-spacing: -0.025em; }
    .pta-number { color: #4f46e5; font-weight: 700; font-family: 'JetBrains Mono', monospace; }
    .kasasi-number { color: #10b981; font-weight: 700; font-family: 'JetBrains Mono', monospace; }
    .label-accent { font-size: 0.65rem; text-transform: uppercase; font-weight: 800; color: #64748b; letter-spacing: 0.05em; margin-bottom: 2px; }
        ...
        <form action="{{ route('kasasi.index') }}" method="GET" id="filterForm">
            <div class="row align-items-center g-3">
                <div class="col-md-3">
                    ...
                    <p class="text-muted small mb-0">Total: <span class="fw-bold text-primary">{{ number_format($grandTotal) }}</span> Berkas</p>
                </div>
                <div class="col-md-4">
                ...
                <div class="card-body p-4">
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-3 border-end">
                            <div class="label-accent">Ketua Majelis Banding</div>
                            <div class="judge-name mb-1"><i class="fas fa-user-tie text-muted me-2 opacity-50"></i>{{ $item->kmh }}</div>
                            <span class="badge badge-soft rounded-pill px-2 py-1"><i class="fas fa-folder-open me-1"></i>{{ $item->jenis_perkara }}</span>
                        </div>
                        {{-- 2. FOKUS NOMOR PTA --}}
                        ...
                        {{-- 3. FOKUS STATUS & NOMOR KASASI --}}
                        <div class="col-lg-4 px-lg-4 border-end">
                            <div class="row align-items-center">
                                <div class="col-7 border-end">
                                    <div class="label-accent">Nomor Kasasi</div>
                                    <div class="kasasi-number small mb-1">{{ $item->no_kasasi }}</div>
                                    <span class="badge bg-{{ $item->status_color }} bg-opacity-10 text-{{ $item->status_color }} rounded-pill px-2 py-1 fw-bold" style="font-size: 0.6rem;">{{ $item->status_label }}</span>
                                </div>
                                <div class="col-5 ps-3">
                                    <div class="label-accent">Keterangan</div>
                                    @if($item->tgl_putusan)
                                        <div class="text-success fw-bold small"><i class="fas fa-check-circle me-1"></i>Putus: {{ \Carbon\Carbon::parse($item->tgl_putusan)->translatedFormat('d M Y') }}</div>
                                    @else
                                        <div class="text-muted small italic">Proses MA</div>
                                    @endif
                                    <div class="text-muted" style="font-size: 0.6rem;">Reg: {{ \Carbon\Carbon::parse($item->tgl_reg_kasasi)->format('d/m/y') }}</div>
                                </div>
                            </div>
                        </div>
                        {{-- 4. AKSI --}}
                        <div class="col-lg-2 text-lg-end">
                            <button class="btn btn-primary rounded-pill py-2 px-4 shadow-sm w-100 fw-bold" data-bs-toggle="modal" data-bs-target="#modal{{ $item->unique_id }}">Isi Amar</button>
                        </div>
                    </div>
                </div>
            </div>
