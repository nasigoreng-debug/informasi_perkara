...
<div class="container-fluid py-4">
    <div class="card shadow border-0 mb-4 animate__animated animate__fadeIn">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Rekap Sisa Panjar {{ $label }} (> 6 Bulan)</h5>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered align-middle">
                    ...
                    <tbody>
                        ...
                        @foreach($data->groupBy('satker_key') as $satker => $group)
                            ...
                            <tr>
                                ...
                                <td class="text-end fw-bold text-primary">Rp {{ number_format($group->sum('sisa'), 0, ',', '.') }}</td>
                                <td class="text-center">
                                    <button class="btn btn-outline-primary btn-sm rounded-pill btn-detail" data-satker="{{ $satker }}">
                    ...
                    <tfoot class="table-secondary fw-bold">
                        <tr>
                            <td colspan="3" class="text-center">GRAND TOTAL SELURUH SATKER</td>
                            <td class="text-end text-primary fs-5">Rp {{ number_format($grandTotalDuit, 0, ',', '.') }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
    ...
    <div id="section-detail" style="display: none;" class="animate__animated animate__fadeInUp">
        <div class="card shadow border-0 border-top border-primary border-4">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-primary fw-bold" id="title-detail">Detail Perkara Satker</h5>
...
{{-- Script JQuery (Wajib ada buat scroll & click) --}}
...
<script>
    ...
    $(document).on('click', '.btn-detail', function() {
        ...
        if (!listPerkara) return;

        $('#title-detail').html('<i class="fas fa-university me-2"></i>Detail Sisa Panjar Perkara Satker ' + satker);
